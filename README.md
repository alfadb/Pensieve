...
npx skills add https://github.com/kingkongshot/Pensieve/tree/experimental/skill-source/pensieve --copy -a claude-code -y
```
...
npx skills add /path/to/Pensieve/skill-source/pensieve -a claude-code --copy
```
...
```bash
npx skills add /path/to/Pensieve/skill-source/pensieve -a codex --copy
```
...
完整的安装、更新、卸载说明见 [skill-lifecycle.md](skill-source/pensieve/.src/references/skill-lifecycle.md)。
...
- `skill-source/pensieve/`：唯一权威 skill 源
- `.claude-plugin/` + `hooks/` + `hooks-handlers/`：Claude 专属薄壳
- `skill-source/pensieve/.src/`：系统规则、脚本、模板
- `skill-source/pensieve/maxims|decisions|knowledge|pipelines|loop/`：skill 模式下的用户数据根
- `<project>/.claude/skills/pensieve/`：Claude plugin 模式下的用户数据根
- `<project>/.state/`：运行时状态、报告、marker、缓存

一句话：

**Claude 和 Codex 可以装得不一样，但它们读的是同一份 `skill-source/pensieve/`。**

### 设计原则

- **单一事实源**：规则、脚本、模板只在 `skill-source/pensieve/` 维护一份
...
仓库根不再充当 skill 根。
根目录是 **plugin/package 根**；真正的 skill 根是 `skill-source/pensieve/`。
使用 `skill-source/` 而非 `skills/` 是为了避免 CC 插件自动发现机制重复加载 skill（CC 会自动扫描插件根的 `skills/*/SKILL.md`）。
