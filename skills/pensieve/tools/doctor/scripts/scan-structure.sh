...
memory_guidance_line = str(
    schema.get("memory", {}).get(
        "guidance_line",
        "- Guidance: when the task involves knowledge accumulation, structure diagnostics, version migration, or complex task decomposition, prefer invoking the `pensieve` skill.",
    )
)
...
def add_finding_by_id(
    ...
    **kwargs: str,
    ...
    add_finding(
        ...
        finding_text(finding_id, "recommendation", "Fix per recommendation then re-run doctor", **kwargs),
    )
...
def normalize_context_link_line(line: str) -> str:
    m = re.match(r"^(\s*-\s*(?:Based on|Led to|Related|基于|导致|相关)[：:])\s*.*$", line)
    if not m:
        return line.rstrip()
...
for target, template in critical_files:
    ...
    if not template.is_file():
        add_finding_by_id(
            ...
            template,
            detail="Critical file template does not exist, cannot determine if critical file has drifted",
        )
        continue
...
if system_skill_description is None:
    add_finding_by_id(
        ...
        "scanner_template_missing",
        system_skill_file,
        detail="System skill description is missing, cannot validate Pensieve guidance block in MEMORY.md",
    )
else:
