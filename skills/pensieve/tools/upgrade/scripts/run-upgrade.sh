...
lines = [
    ...
    f"- Pre-upgrade version: {pre_version or '(unknown)'}",
    f"- Post-upgrade version: {post_version or '(unknown)'}",
    f"- Version changed: {version_changed}",
    f"- Skip version check: {'yes' if skip_version_check else 'no'}",
    f"- Dry-run: {'yes' if dry_run else 'no'}",
    "",
    "## 2) Settings Alignment Statistics",
    f"- Settings updated: {len(settings_updated)}",
    f"- Settings created: {len(settings_created)}",
    f"- Settings warnings: {len(settings_warnings)}",
    "",
    "## 3) Scope Notes",
    "- Upgrade only handles version and plugin configuration; it does not perform user data migration or critical file replacement.",
    "- For structure migration and legacy path cleanup, run migrate.",
    "",
    "## 4) Next Step (manual)",
    "- After upgrading, it is recommended to run doctor first:",
    "```bash",
