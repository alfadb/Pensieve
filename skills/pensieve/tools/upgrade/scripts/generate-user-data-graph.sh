...
}
{
    ...
    line = $0
    label = "link"
    if (index(line, "Based on") > 0 || index(line, "基于") > 0) label = "based-on"
    else if (index(line, "Led to") > 0 || index(line, "导致") > 0) label = "led-to"
    else if (index(line, "Related") > 0 || index(line, "相关") > 0) label = "related"
...
END {
    ...
    print ""
    print "# Pensieve User Data Graph (Auto-Generated)"
    print ""
    print "> This file is auto-generated by a script; no manual maintenance is required."
    print ""
    print "- Root: `" root "`"
    ...
    #   A --based-on--> B and B --led-to--> A keep only "based-on".
    for (i = 1; i <= edge_unique_count; i++) {
        keep_edge[i] = 1
        if (edge_unique_lbl[i] == "led-to") {
    ...
    print "- Resolved links: " resolved
    print "- Unresolved links: " unresolved
