...
def _parse_graph(path: Path) -> tuple[dict[str, int], list[tuple[str, str]]]:
    ...
    patterns = {
        ...
        "resolved": re.compile(r"^- Resolved links:\s*(\d+)\s*$", flags=re.MULTILINE),
        "unresolved": re.compile(r"^- Unresolved links:\s*(\d+)\s*$", flags=re.MULTILINE),
    }
...
def _build_report(
    *,
    ...
    if graph_stats["unresolved"] == 0:
        graph_observation = "All graph links are resolvable."
    else:
        hard = sum(1 for src, _ in unresolved_links if src.startswith("decisions/") or src.startswith("pipelines/"))
        graph_observation = f"Found {graph_stats['unresolved']} unresolved links ({hard} broken decision/pipeline links)."
    ...
    lines.append(f"- Data directory: `{user_root}`")
    lines.append("")
    lines.append("## 1) Executive Summary (start here)")
    ...
    lines.append("")
    lines.append("## 1.5) Graph Summary (evidence for conclusions)")
    ...
    lines.append(f"- Resolved links: {graph_stats['resolved']}")
    lines.append(f"- Unresolved links: {graph_stats['unresolved']}")
    lines.append(f"- Graph observation: {graph_observation}")
    lines.append("")
    lines.append("## 2) Priority Items (MUST_FIX, by priority)")
    if not must_fix:
        lines.append("- (none)")
    else:
    ...
    if lines and lines[-1] == "":
        lines.pop()
    lines.append("")
    lines.append("## 3) Recommended Fixes (SHOULD_FIX)")
    if not should_fix:
        lines.append("- (none)")
    else:
    ...
    lines.append(f"- Legacy paths found: {_yes_no(flags.get('has_deprecated_paths'))}")
    lines.append(f"- Old and new coexist: {_yes_no(flags.get('has_deprecated_paths'))}")
    lines.append(f"- Non-project-level skill root found: {_yes_no(flags.get('has_deprecated_paths'))}")
    lines.append(f"- Standalone graph files found: {_yes_no(flags.get('has_legacy_graph_files'))}")
    lines.append(f"- Missing critical directories: {_yes_no(flags.get('has_missing_directories'))}")
    lines.append(
        f"- MEMORY.md missing/drifted: {_yes_no(flags.get('has_missing_memory_file') or flags.get('has_memory_content_drift'))}"
    )
    lines.append(f"- Recommended action: `{next_step if next_step in {'migrate', 'upgrade', 'self-improve'} else 'none'}`")
    lines.append("")
    lines.append("## 5) Three-Step Action Plan")
    if must_fix and has_migrate_must_fix:
        lines.append("1. Run the `migrate` script first to complete structure migration and critical file alignment.")
        lines.append("2. Run `doctor` again to confirm MUST_FIX count is zero.")
        lines.append("3. If SHOULD_FIX/INFO items remain, fix them by priority.")
    elif must_fix and has_upgrade_must_fix:
        lines.append("1. Run the `upgrade` script first to complete version update and plugin key alignment.")
        lines.append("2. Run `doctor` again to confirm MUST_FIX count is zero.")
        lines.append("3. If structure migration issues arise later, run `migrate`.")
    elif must_fix:
        lines.append("1. Fix MUST_FIX items per this report (frontmatter/broken links/MEMORY content issues); no need to run `migrate` first.")
        lines.append("2. Re-run `doctor` after fixes to confirm MUST_FIX count is zero.")
        lines.append("3. If migration issues arise later (legacy paths/critical file drift), run `migrate`.")
    elif should_fix or info:
        lines.append("1. Address SHOULD_FIX items first to ensure long-term maintainability.")
        lines.append("2. Re-run `doctor` after fixes to confirm status becomes PASS.")
        lines.append("3. Record effective fix patterns via `self-improve`.")
    else:
        lines.append("1. No structural fixes needed at this time.")
        lines.append("2. Continue using loop/self-improve as usual.")
        lines.append("3. Run doctor again after the next upgrade.")
    lines.append("")
    lines.append("## 6) Rule Hit Details (Appendix)")
    lines.append("| ID | Severity | Category | File/Path | Rule Source | Issue | Fix Recommendation |")
    lines.append("|---|---|---|---|---|---|---|")
    if findings:
        for f in findings:
        ...
        lines.append("| - | - | - | - | - | - | - |")
    lines.append("")
    lines.append("## 7) Graph Broken Links Details (Appendix)")
    ...
    lines.append("")
    lines.append("## 8) Frontmatter Quick Check Results (Appendix)")
    ...
    for item in scan.get("findings", []):
        ...
        findings.append(
            Finding(
                ...
                recommendation=str(item.get("recommended_action", "Fix per recommendation then re-run doctor")),
            )
        )
    ...
    for idx, (src, target) in enumerate(unresolved_links, start=1):
        ...
        findings.append(
            Finding(
                ...
                recommendation="Add the target file or correct the link target name",
            )
        )
